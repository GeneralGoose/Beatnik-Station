...
/obj/machinery/field/containment/Crossed(obj/mover)
	if(istype(mover, /obj/machinery) || istype(mover, /obj/structure) || istype(mover, /obj/mecha))
		spawn(1) //stops things from bouncing between 2 force fields 9999 times a second creating billions of sparks to lag the server to death. a delay in the bump_field itself only affects mobs for some fucking reason
			bump_field(mover)
...
/obj/machinery/field/proc/bump_field(atom/movable/AM as mob|obj)
	...
	if(AM.last_bumped_field == 0) //first bump ever
		...
		if(world.time - AM.last_bumped_field < 50 && AM.times_bumped_field > 20) //if something bounced more than 20 times and it's been less than 5 seconds since last bounce, alert admins
			...
			message_admins("Warning! [AM.name] is bouncing off the containment field too often in <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[AM.x];Y=[AM.y];Z=[AM.z]'>[A.name]</a>! Might be a shitter trying to lag the server. There will be no more alerts for this object. You better check it out, bro!")
